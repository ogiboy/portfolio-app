...
4. Root `DESIGN.md` defines the current visual and interaction contract.
5. `.ai/roadmap/` records future sequence.
6. `.ai/checkpoints/portfolio-overhaul.md` records the sole mutable state of the active overhaul.
7. `docs/aegis/` records dated initiative intent, plans, snapshots, and evidence.
8. `package.json` defines local commands.
9. CI enforces the remote verification contract.
10. `SECURITY.md` defines vulnerability reporting.
11. Supporting `.ai` capability, workflow, security, memory, development, and versioning instructions
    route work beneath the owners above.
12. CodeRabbit and other automated reviewers are advisory only and do not override any item above.
...
- Run application commands with Node `24.16.0` from `.nvmrc`/`.node-version`; separately installed
  OMX/Ruflo tools may use their own host Node without changing the app contract.
- Do not rewrite history, reset user work, deploy, publish, change DNS, or use credentials without explicit authority.
- Keep local runtime artifacts out of commits: `.env.local`, `.vercel`, `.next`, `.omx`, `.swarm`,
  `.claude`, `.claude-flow`, `.codex/config.toml`, `.codex/hooks.json`, `agentdb.rvf`,
  `agentdb.rvf.lock`, `ruvector.db`, `.DS_Store`.
- Preserve the reviewed project-local OMX surface in `.codex/agents`, `.codex/prompts`, and
  `.codex/skills`; do not hand-edit the generated section between the OMX markers below.
...
- Keep skills, agents, prompts, workflows, MCPs, plugins, OMX, and Ruflo as development-time
  capabilities. They are not Next.js runtime services or alternate product-state owners.
...
- Before any push or release, run the exact full package gate: `pnpm run ci`.
