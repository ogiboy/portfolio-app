...
Move first-party DOS runtime behavior out of `public/wasm/engine` and into modular typed source:
Next/React owns the parent experience, and a browser-compatible entry owns the isolated engine
document. Preserve the current public game-center contract. The browser must continue to load the
matched generated Emscripten glue/WASM pair and the selected game files from static public URLs.
The migration changes ownership and testability, not the DOS emulator build.
...
- Current manifest runtime revision: `doswasmx-v0.3-hot.2`.
...
Compatibility decision: `/wasm/engine/index.html` remains the stable, same-origin sandbox and
open-separately entrypoint. The current file is a hand-authored shell that loads `romlist.js` and
`settings.js`; after cutover it must instead be a deterministic static compatibility artifact emitted
from a browser-compatible, typed source entry. It must not redirect or load a Next route: the
opaque-origin iframe cannot depend on the parent React tree or a Next server-rendered module at
runtime. The generated entry may contain only the document root and bootstrap wiring needed to load
its generated browser bundle. Parent lifecycle remains React-owned, while the compatibility bundle
owns only the engine-document DOM adapter and reports through the typed protocol; neither side may
implement a second lifecycle, asset-selection, or recovery policy.
...
| Parent lifecycle and parent-facing UI                                  | React-owned client leaf in `src/components/client/wasm-runtime/`                   | Parent state, controls, and cleanup stay outside the opaque-origin frame                                      |
| Engine-document DOM adapter and engine mount                           | Typed browser-compatible compatibility entry and deterministic emitter             | Generated static bundle only; no hand-authored shell implementation remains under `public/wasm/engine`        |
| Compatibility frame entrypoint                                         | Browser-compatible entry source plus deterministic emitter in `scripts/wasm/`      | `/wasm/engine/index.html` stays a generated bootstrap artifact; it neither redirects nor imports a Next route |
...
- No untyped `window` protocol payloads, `eval`, jQuery, Rivets, toastr, or hidden retry. The only
  ...
  `myApp` ABI; no product state may escape through a second global owner. The generated static
  compatibility bundle is not charged to authored-source budgets; its typed source entry is.
...
4. The generated public tree must contain no hand-authored runtime shell implementation after the
   ...
   compatibility bootstrap and its declared browser bundle, retain the pinned engine/support and
   license artifacts, and reject stale wrapper files, duplicate protocol implementations, and legacy
   vendor dependencies.
...
- Inventory every current public shell responsibility in `index.html`, `runtime-security.js`,
  `input_controller.js`, `script.js`, `settings.js`, and `romlist.js`, including DOM nodes, event
  listeners, globals, security helpers, IndexedDB reads, canvas setup, engine callbacks, notices,
  asset URLs, dynamic script order, and `postMessage` paths. Separately inventory retained engine
  support/provenance artifacts (`main.js`, `main.wasm`, `main.ttf`, configuration, image, notices,
  and GPL text) so the cutover test does not mistake them for authored shell code.
...
- Add characterization tests before moving behavior. The baseline must pass the existing
  `tests/wasm-delivery.test.ts`, `tests/wasm-game-frame.test.tsx`,
  `tests/wasm-script-security.test.ts`, `tests/wasm-settings-security.test.ts`, and the WASM
  journeys in `e2e/smoke.spec.ts`.
...
### WASM-2 — Parent shell and compatibility-entry cutover

- Move parent controls, status UI, focus, cleanup, and explicit boot into the React-owned client
  leaf; keep the engine-document adapter in the browser-compatible compatibility entry.
- Add a deterministic emitter for `/wasm/engine/index.html` and its browser bundle. Its input must
  be the typed compatibility entry, not a Next route or runtime import from the parent React tree.
  Its output must contain no independent parent lifecycle, asset-selection, or recovery policy.
...
- Remove only the now-unused authored shell files from `public/wasm/engine`; retain the generated
  compatibility `index.html` and browser bundle, `main.js`, `main.wasm`, support artifacts,
  manifest/delivery records, and all game payloads.
...
The migration is accepted only when the following requirements are met. They are planned acceptance
requirements, not claims that the listed checks have already run:

- Existing `tests/wasm-delivery.test.ts` must verify static ownership, no route handler/vendor tree,
  exact manifest revision, both pinned digests, security/cache/MIME headers, no legacy wrapper
  dependencies, required generated compatibility `index.html` and bundle, and generated-artifact
  inventory.
...
- Existing `tests/wasm-script-security.test.ts` and `tests/wasm-settings-security.test.ts` must be
  retained or replaced with equivalent focused coverage for the compatibility bundle's asset,
  origin, message, dynamic-load-order, and security boundaries.
- A new `tests/wasm-runtime-source.test.ts` must verify source ownership, typed protocol
  validation, reducer and asset-boundary behavior, modularity budgets, and absence of authored shell
  code in public.
- WASM cases in `e2e/smoke.spec.ts`: zero `/wasm/*` requests before intent; successful boot loads
  ...
  open-separately entrypoint without redirecting or requesting a Next route; `application/wasm`,
  CSP, sandbox, timeout/404 recovery, retry, IndexedDB-unavailable boot, localized EN/TR routes,
  mobile no-overflow, and no page/console errors.
...
- Roll back by restoring the last known-good source compatibility entry and its emitted public
  artifacts as one matched commit. Never roll back only `main.js`, only `main.wasm`, only the
  manifest revision, or only the wrapper.
...
- Keep the old source implementation and its generated output available only during a bounded
  local/preview compatibility window. The public compatibility URL remains
  `/wasm/engine/index.html`; remove the old source/output together after all gates pass and do not
  maintain two permanently divergent implementations.

## Done criteria

- Parent DOS shell behavior is owned by small, typed Next/React modules, and engine-document
  behavior is owned by a small, typed browser-compatible entry with no duplicate public
  implementation.
...
- Rollback is documented, browser-checked at the same compatibility URL against the prior emitted
  artifact, and does not require changing the generated engine pair independently.
